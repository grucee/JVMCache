...
public class ClassHelper {
	...
	/**
	 * 优先使用线程上线文类加载器，找不到再使用调用者的类加载器，最后找不到返回空
	 * 
	 * @param name
	 * @param caller
	 * @return
	 */
	public static Class<?> getClass(String name, Class<?> caller) {
		Class<?> clazz = null;
		try {
			clazz = forNameWithThreadContextClassLoader(name);
		} catch (ClassNotFoundException e) {
		}

		if (clazz == null) {
			try {
				clazz = forNameWithCallerClassLoader(name, caller);
			} catch (ClassNotFoundException e) {
			}
		}
